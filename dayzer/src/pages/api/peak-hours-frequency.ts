...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    // Analyze forecast week: find top 3 hours for each day
    ...
    for (let day = 0; day < 7; day++) {
      ...
      // Sort by LMP and get top 3
      const top3Hours = dayHours
        .sort((a, b) => b.lmp - a.lmp)
        .slice(0, 3)
        .map(d => d.hour);
      
      // Increment frequency for each top 3 hour
    ...
    // Analyze last week: find top 3 hours for each day
    ...
    console.error('Peak hours frequency error:', error);
    return new Response(
      JSON.stringify({ error: 'Failed to fetch peak hours frequency data' }),
